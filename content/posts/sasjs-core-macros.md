...
Every SAS developer eventually writes the same utility macro twice - once because they needed it, and again in the next project because nobody could find the first one. [SASjs Core](https://github.com/sasjs/core) is the library that stops that happening: over 250 fully documented macros covering everything from dataset inspection and metadata lookups to Viya REST calls, all [MIT licensed](https://github.com/sasjs/core/blob/main/LICENSE) and free for commercial use. This post is a short tour of what is in it, and the five ways to get it into your environment.

## What is in the library

The macros are grouped into nine folders, by the platform they target and the job they do:

* **[BASE](https://github.com/sasjs/core/tree/main/base)** - macro functions and procedures that run on any SAS platform, no X command required (`mf_`, `mp_`)
* **[DDL](https://github.com/sasjs/core/tree/main/ddl)** - creating and changing tables (`mddl_`)
* **[FCMP](https://github.com/sasjs/core/tree/main/fcmp)** - macros that generate `proc fcmp` functions (`mcf_`)
* **[LUA](https://github.com/sasjs/core/tree/main/lua)** - LUA modules wrapped as macros, for the `proc lua` interpreter (`ml_`)
* **[META](https://github.com/sasjs/core/tree/main/meta)** - metadata server access on SAS 9 and Enterprise BI (`mm_`)
* **[METAX](https://github.com/sasjs/core/tree/main/metax)** - the metadata macros that need an enabled X command (`mmx_`)
* **[SERVER](https://github.com/sasjs/core/tree/main/server)** - for apps running on [SASjs Server](https://server.sasjs.io) (`ms_`)
* **[VIYA](https://github.com/sasjs/core/tree/main/viya)** - interfacing with SAS Viya (`mv_`, `mvf_`)
* **[XPLATFORM](https://github.com/sasjs/core/tree/main/xplatform)** - macros written to behave the same on Viya, SAS 9 and SASjs Server (`mx_`)

Each macro carries its documentation in a [doxygen header](https://github.com/sasjs/core#header-properties) - parameters, return values, and the related macros you probably want next. Those same comments generate the [published documentation](https://core.sasjs.io), so the reference and the code cannot drift apart.
...
**2. Compile the whole set in two lines.** If your SAS session has internet access, the build generates a [single file](https://github.com/sasjs/core/blob/main/all.sas) containing every macro:
...
**3. Install it as an npm package.** JavaScript and CLI projects pull the macros in from [npm](https://www.npmjs.com/package/@sasjs/core) and pin them to a version, the same way as any other dependency:

```bash
npm install @sasjs/core
```

That writes the library into `node_modules/@sasjs/core` - the same nine folders as the repo, in the project and ready to be brought into the build.
...
```sas
/* a folder for the packages - the fileref must be named "packages", */
/* and the folder must already exist (WORK is the portable choice) */
filename packages "%sysfunc(pathname(work))";

/* the framework is a single file */
...
/* optional: print the package help to the log */
%helpPackage(sasjscore)

/* compile the macros into the session */
